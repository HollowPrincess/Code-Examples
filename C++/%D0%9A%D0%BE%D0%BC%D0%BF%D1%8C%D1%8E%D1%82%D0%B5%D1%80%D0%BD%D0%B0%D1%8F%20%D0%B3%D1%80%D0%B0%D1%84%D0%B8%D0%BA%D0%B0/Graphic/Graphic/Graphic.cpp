...
void CohenSutherland(int& x1, int& y1, int& x2, int& y2) {
	...
	while ((codeA | codeB) && (!(codeA&codeB))) {
		...
		if (codeA & 1) {
			...
			if (A[0] != B[0] || A[1] != B[1]) {
				A[0] += ceil((B[0] - A[0])*(borderBottom - A[1]) / (B[1] - A[1]));
				A[1] = borderBottom;
			}
			codeA -= 4;
		}
...
void BrezenghemLineI(int d1, int d2, int some1, int some2, int limit, TGAImage &image, TGAColor somecolor, bool isXfirst, bool isDotInFirstQuadrant) {
	int d = 2 * d2 - d1;
	do {
		if (isXfirst) {
...
void  line(int x1, int y1, int x2, int y2, TGAImage& image, TGAColor somecolor) {
	...
	/*if (y1 <= y2 && x1 >= x2) {
	if (dx >= dy) {
	BrezenghemLineI(dx, dy, x, y, x2, image, somecolor, true,false);
	}
	else {
	BrezenghemLineI(dy, dx, y, x, y2, image, somecolor,false,false);
	};
	}*/
	if (y1 <= y2 && x1 < x2) {
		if (dx>dy) {
			BrezenghemLineI(dx, dy, x, y, x2, image, somecolor, true, true);
		}
		else {
			BrezenghemLineI(dy, dx, y, x, y2, image, somecolor, false, true);
		};
	}
	else {
		if (dx >= dy) {
